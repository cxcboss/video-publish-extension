...
**自动更新检测**
- 启动时自动检测 GitHub 最新版本，有新版本时刷新按钮变绿提醒
- 点击刷新按钮手动检测
- 弹出更新面板：显示版本号、更新日志
- 点击"更新"按钮：自动下载 ZIP 到本地，打开扩展管理页面

### v2.3.1 (2026-07-03)

**视频号上传等待优化**
- `waitForUploadComplete` 最大等待从 60 秒改为 8 秒，超时后继续填写表单，不再死等

**超时自动重试**
- 多视频排队发布时，单个视频任务超时 30 秒自动关闭标签页并重试
- UI 新增"超时自动重试"开关（位于定时发布和 AI 生成之间），支持设置重试次数（默认 1 次）
- 超过最大重试次数则跳过该视频继续下一个

**点击图标直接打开侧边栏**
- 添加 `chrome.sidePanel.setPanelBehavior({ openPanelOnActionClick: true })`，点击扩展图标直接打开侧边栏

...
class _HomePageState extends State<HomePage> with WindowListener {
  ...
  Future<void> _initTray() async {
    ...
    await _tray!.initSystemTray(title: '视频发布助手', iconPath: iconPath);
    ...
    await menu.buildFrom([
      ...
      MenuSeparator(),
      MenuItemLabel(label: '退出', onClicked: (_) async { await _doQuit(); }),
    ]);
  ...
  Future<void> _checkStatus() async {
    try {
      final r = await http.get(Uri.parse('http://127.0.0.1:3000/health')).timeout(const Duration(seconds: 2));
      if (r.statusCode == 200) {
  ...
  Future<void> _startServer() async {
    ...
    if (nodePath.isEmpty) { if (mounted) setState(() { serverError = '未找到 Node.js'; }); return; }
    if (!File(p.join(serverDir, 'server.js')).existsSync()) {
      if (mounted) setState(() { serverError = 'server.js 不存在'; }); return;
    }
  ...
  Future<void> _installDeps() async {
    ...
    try {
      ...
      if (result.exitCode == 0) { _showToast('✓ 服务依赖安装完成'); _refreshEnv(); }
      else { _showToast('✗ 安装失败 (exit=${result.exitCode})'); }
    } catch (e) { _showToast('✗ 安装失败: $e'); }
  }

  Future<void> _installExtension() async { _showToast('✓ 插件已就绪'); _refreshEnv(); }
  ...
  Future<void> _autoCheckUpdate() async {
    _refreshEnv();
    try {
      final r = await http.get(Uri.parse('https://api.github.com/repos/cxcboss/video-publish-extension/releases/latest')).timeout(const Duration(seconds: 10));
      if (r.statusCode == 200) {
        ...
        for (var i = 0; i < 3; i++) {
          ...
          if (l > c) { hasUpdate = true; break; } if (l < c) break;
        }
        if (hasUpdate && mounted) {
          setState(() { updateInfo = {'installed': extVersion, 'latest': latest, 'changelog': j['body'] ?? '', 'zipUrl': (j['assets'] as List?)?.isNotEmpty == true ? j['assets'][0]['browser_download_url'] : ''}; });
        }
      }
    } catch (_) {}
  }
  ...
  Future<void> _doUpdate() async {
    ...
    try {
      ...
      final extractResult = await Process.run('powershell.exe', ['-NoProfile', '-Command', 'Expand-Archive -Path "$tmpZip" -DestinationPath "$tmpExtract" -Force']);
    ...
    return Scaffold(
      body: Stack(
        children: [
            ...
            child: ListView(
              children: [
                Row(children: [
                  ...
                  const Spacer(),
                  Text('v${extVersion.isEmpty ? "2.5.2" : extVersion}', style: const TextStyle(fontSize: 11, color: Colors.grey)),
                ]),
                ...
                _card(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  if (serverError.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 4), child: Text(serverError, style: const TextStyle(fontSize: 11, color: Colors.red))),
                  const SizedBox(height: 8),
                  if (serverStarting)
                    const Row(children: [SizedBox(width: 14, height: 14, child: CircularProgressIndicator(strokeWidth: 2)), SizedBox(width: 8), Text('启动中...', style: TextStyle(fontSize: 13, color: Colors.grey))])
                  else
                ...
                _card(child: Row(children: [
                  ...
                  const Spacer(),
                  OutlinedButton(onPressed: _installDeps, child: Text(envInstalled ? '重新安装' : '安装环境')),
                ])),
                ...
                _card(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    ...
                    const SizedBox(width: 6),
                    Text(extInstalled ? '插件已安装 (v$extVersion)' : '插件未安装', style: const TextStyle(fontSize: 13)),
                    const Spacer(),
                ...
                _card(child: Row(children: [
                  ...
                  const Spacer(),
                  Switch(value: autoStart, onChanged: (v) { autoStart = v; _toggleAutoStart(); }),
                ])),
          ...
          if (toast.isNotEmpty) Positioned(bottom: 16, left: 16, right: 16,
            child: Material(color: Colors.white12, borderRadius: BorderRadius.circular(8),
              child: Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), child: Text(toast, style: const TextStyle(fontSize: 12))))),
        ],
      ),
    );
  }
  ...
  Widget _buildUpdateSection() {
    if (updateInfo != null && (updateInfo!['latest'] ?? '').isNotEmpty) {
      return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('发现新版本: v${updateInfo!['installed']} → v${updateInfo!['latest']}', style: const TextStyle(fontSize: 13, color: Colors.green)),
        const SizedBox(height: 4),
        Container(width: double.infinity, constraints: const BoxConstraints(maxHeight: 80), padding: const EdgeInsets.all(8),
          decoration: BoxDecoration(color: Colors.white10, borderRadius: BorderRadius.circular(4)),
          child: SingleChildScrollView(child: Text(updateInfo!['changelog'] ?? '', style: const TextStyle(fontSize: 11, color: Colors.grey)))),
        const SizedBox(height: 8),
        if (updateInfo!['updating'] == true) const LinearProgressIndicator()
        else Row(children: [
    ...
    return Row(children: [
      if (checkingUpdate) const SizedBox(width: 14, height: 14, child: CircularProgressIndicator(strokeWidth: 2)),
      if (checkingUpdate) const SizedBox(width: 8),
      if (!checkingUpdate) OutlinedButton(onPressed: _checkUpdate, child: const Text('检测更新')),
    ]);
  }

  Widget _card({required Widget child}) {
    return Card(margin: const EdgeInsets.only(bottom: 12), child: Padding(padding: const EdgeInsets.all(14), child: child));
  }
}
